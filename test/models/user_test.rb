...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid adresses" do
    valid_adddresses = %w[user@example.com USER@foo.COM A_US-ER@foo.bar.org first.last@foo.jp alice+bob@baz.cn]
    valid_adddresses.each do |valid_adddresse|
      @user.email = valid_adddresse
      assert @user.valid?, "#{valid_adddresse.inspect} should be valid"
    end
  end
